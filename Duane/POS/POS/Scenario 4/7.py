...
time.sleep(1)
driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-masterfiles/ion-content/div[2]/ion-grid/ion-row/ion-col/ion-item[3]/div[1]/div/ion-label').click()
time.sleep(1)
sc = driver.find_element_by_xpath('/html/body/ion-app/ion-modal/div/page-syspars-crud/ion-content/div[2]/form/ion-card/ion-grid/ion-row[1]/ion-col/ion-item/div[1]/div/ion-input/input').get_attribute('value')
time.sleep(1)
if sc == "10":
    driver.find_element_by_xpath('/html/body/ion-app/ion-modal/div/page-syspars-crud/ion-content/div[2]/form/ion-card/ion-grid/ion-row[2]/ion-col/button[1]/span').click()
else: 
    driver.find_element_by_xpath("/html/body/ion-app/ion-modal/div/page-syspars-crud/ion-content/div[2]/form/ion-card/ion-grid/ion-row[1]/ion-col/ion-item/div[1]/div/ion-input/input").send_keys(Keys.BACKSPACE)
    time.sleep(1)
    driver.find_element_by_xpath('/html/body/ion-app/ion-modal/div/page-syspars-crud/ion-content/div[2]/form/ion-card/ion-grid/ion-row[1]/ion-col/ion-item/div[1]/div/ion-input/input').send_keys(10)
    time.sleep(1)
    driver.find_element_by_xpath('/html/body/ion-app/ion-modal/div/page-syspars-crud/ion-content/div[2]/form/ion-card/ion-grid/ion-row[2]/ion-col/button[2]/span').click()
    time.sleep(1)
...
driver.switch_to.alert.accept()
time.sleep(2)
driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-orders/ion-content/div[2]/ion-tabs/ion-tab[1]/page-table-view[2]/ion-content/div[2]/div/button[2]/span').click()
time.sleep(1)
...
while i < int(length):
    ...
    if items[i].get_attribute('innerText') == "ADOBO":
    ...
    if items[i].get_attribute('innerText') == "NILAGA 'VAT E\"":
        nilagaVatE += 1      
    if items[i].get_attribute('innerText') == "SINIGANG":
        p = price[(i*2)+2].get_attribute('innerText')
        iwv = p[2:]
        iwv_price.append(float(iwv))
        sinigang += 1    
...
service_charge_discount = round(float(float(senior) * float(int(sc) / 100)),2)
service_charge = round(float(float((float(sum(iwv_price)) / 1.12) * float(int(sc) / 100)) + float(float(item_wout_vat_price) * float(int(sc) / 100)) - float(service_charge_discount)),2)
Ftotal = round(float(float(sum(total)) + float(service_charge) - float(vat_adjustment) - float(senior)),2)
...
vat_exempt_sales = round(float(float(sales_without_vat) + float(service_charge)),2)
...
pos_service_charge_discount = vat[i-2].get_attribute('innerText')
pos_service_charge = vat[i].get_attribute('innerText')
pos_Ftotal = vat[i+2].get_attribute('innerText')
pos_vatable_sales = vat[i+4].get_attribute('innerText')
pos_vat_amount = vat[i+6].get_attribute('innerText')
pos_vat_exempt_sales = vat[i+8].get_attribute('innerText')
...
pos_service_charge_discount = pos_service_charge_discount.replace(',','')
pos_service_charge = pos_service_charge.replace(',','')
...
pymsgbox.alert('Service Charge Discount: ' + str(service_charge_discount) + '\n' + 'POS Service Charge Discount: ' + str(pos_service_charge_discount), timeout = 3000)
if float(service_charge_discount) == float(pos_service_charge_discount):
    pymsgbox.alert('Service Charge Discount Match ✅',timeout = 3000)
else:
    pymsgbox.alert('Service Charge Discount Not Match ❌',timeout = 3000)  

time.sleep(0.5)

pymsgbox.alert('Service Charge: ' + str(service_charge) + '\n' + 'POS Service Charge Discount: ' + str(pos_service_charge), timeout = 3000)
if float(service_charge) == float(pos_service_charge):
    pymsgbox.alert('Service Charge Match ✅',timeout = 3000)
else:
