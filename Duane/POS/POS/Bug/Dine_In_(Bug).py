...
time.sleep(1)
driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-orders/ion-content/div[2]/ion-tabs/div[1]/a[2]/span').click()
time.sleep(1)
...
label = driver.find_element_by_class_name('start').get_attribute('innerText')
di = "DINE-IN"
to = "TAKE OUT"

if label.find(di) != -1 or label.find(to) != -1:
    pymsgbox.alert('PASSED ✅',timeout=3000)
else:
    pymsgbox.alert('MUST INDICATE WHAT TYPE OF ORDER IT IS  (DINE IN / TAKE OUT) ❌') 
    time.sleep(1)