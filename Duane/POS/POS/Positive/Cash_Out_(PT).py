...
for i in range(0,2):
    ...
    if int(value) <= 0:
        driver.execute_script('alert("CANT CASH OUT IF VALUE IS 0: ✅ ");')
        time.sleep(5)
        driver.switch_to.alert.accept()
        time.sleep(2)
    ...
    driver.find_element_by_xpath('/html/body/ion-app/ion-alert/div/div[3]/div/div/input').send_keys("Buy something")
    time.sleep(2)
    ...
    driver.switch_to.alert.accept()
    time.sleep(2)
    driver.execute_script('alert("ALL BUTTONS IS WORKING PROPERLY: ✅" +"\\n" +"CANT CASH OUT IF VALUE IS 0: ✅" +"\\n" +"CANT CONFIRM IF REASON IS EMPTY: ✅" +"\\n" +"CASH OUT HAS BEEN SUCCESSFULLY SAVE: ✅ ");')           
    time.sleep(5)
    driver.switch_to.alert.accept()
    time.sleep(2)
    ...
    print(x)
    if int(co) == int(x):
        driver.execute_script('alert("CASH OUT VALUE MATCH ✅");')
    else:
        driver.execute_script('alert("CASH OUT VALUE NOT MATCH ❌");')
