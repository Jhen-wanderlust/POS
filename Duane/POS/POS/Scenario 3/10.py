...
driver.switch_to.alert.accept()
time.sleep(2)
driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-orders/ion-content/div[2]/ion-tabs/ion-tab[1]/page-table-view[2]/ion-content/div[2]/div/button[4]/span').click()
time.sleep(1)
...
item_wout_vat_price = round(float(sum(total)) - float(sum(iwv_price)),2) #630
pax_vatable = round(float(sum(iwv_price) / int(seat)),2) #130
sales_without_vat = round(float((pax_vatable) / 1.12) + float(item_wout_vat_price),2) # 746
vat_adjustment = round(float(pax_vatable) -  (float(sales_without_vat) - float(item_wout_vat_price)),2)   #13.93 
pax_without_vat = round(float(item_wout_vat_price / int(seat)),2) # 126
senior = round((float((pax_vatable / 1.12) + pax_without_vat) * 0.2),2)  #48.41
Ftotal = round((float(sum(total)) - float(vat_adjustment) - float(senior)),2) #1217.66
