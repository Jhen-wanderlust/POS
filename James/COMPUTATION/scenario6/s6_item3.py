...
time.sleep(1)
caldereta = driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-order-preview/ion-content/div[2]/div/div[2]/div/div[6]/div/h4[2]').get_attribute('innerText')
time.sleep(1)
...
print('price',fish_fillet)
print('price',caldereta)
print('price',nilaga)
print('price',st)

#subtotal computation
subtotal =  float (fish_fillet) + float (caldereta) + float (nilaga)
...
time.sleep(1)
sc = 0.10
vatable = (float (caldereta ) + float (fish_fillet)) / 1.12 * sc
string_vat1 = (np.round(vatable, 2))

#vat exempt computation
vat_exempt = float(nilaga) * sc

srvc_chrg= (string_vat1)+ vat_exempt
srvc_chrg1 = (np.round(srvc_chrg, 2))
if float(sc_total) == srvc_chrg1:
...
print('vat exempt computation',srvc_chrg1)  
...
driver.execute_script('alert("ITEM 3 COMPUTATION IN SCENARIO 6 PASSED✅");')
time.sleep(4)
driver.switch_to.alert.accept() 
time.sleep(4) 
