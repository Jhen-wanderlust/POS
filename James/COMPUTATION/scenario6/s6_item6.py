...
time.sleep(1)
sinigang = driver.find_element_by_xpath('/html/body/ion-app/ng-component/ion-split-pane/ion-nav/page-order-preview/ion-content/div[2]/div/div[2]/div/div[7]/div/h4[2]').get_attribute('innerText')
time.sleep(1)
...
subtotal =  float (adobo) + float (sisig) + float (sinigang)
...
time.sleep(1)
sc = 0.10
vatable = (float (adobo ) + float (sinigang)) / 1.12 * sc
string_vat1 = (np.round(vatable, 2))

#vat exempt computation
vat_exempt = (float(sisig) ) * sc
...
driver.execute_script('alert("ITEM 6 COMPUTATION IN SCENARIO 6 PASSED✅");')
time.sleep(4)
driver.switch_to.alert.accept() 
time.sleep(4) 
