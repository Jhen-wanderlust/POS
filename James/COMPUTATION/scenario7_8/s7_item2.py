...
subtotal =  float (memc_combo_1) + float (memc_combo_3)
...
#SALES WITHOUT VAT
#MEMC ITEMS/ 1.12 + VAT EXEMPT ITEMS 
...
time.sleep(1)
vat_item = (float(st)/1.12)*sc
summation= vat_item - float(sc_disc)
summation2 = (np.round(summation,2))


if float(service_charge) == summation:
...
print('TOTAL computation',total2)

driver.execute_script('alert("ITEM 2 IN SCENARIO 7 & 8 PASSED!✅ ");')
time.sleep(4)
driver.switch_to.alert.accept() 
time.sleep(4)